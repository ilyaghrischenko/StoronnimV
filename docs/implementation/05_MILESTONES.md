...
- **Цель:** доказать сборку и один public vertical на воспроизводимом локальном test corpus.
...
- **Демонстрация:** Home/News получают локальные тестовые данные через настроенный API.
- **Блокеры:** доступные local PostgreSQL/Azurite и toolchain; production backup не блокирует M1.
- **Принятие:** другой сеанс повторяет запуск без догадок и production mutation.
- **Можно отложить:** admin, mobile polish, production hosting и перенос реального content/media до M5.
...
- **Принятие:** desktop checklist проходит на локальном test corpus без P0/P1 defects.
...
- **Вход:** M4 принят; hosting/access и источник production content подтверждены.
- **Выход:** CI, environment contract, импорт и backup real content, migration rehearsal, rollback, deployment и актуальные runbooks.
- **Демонстрация:** staging/rehearsal deploy с выбранным real content source тем же процессом, затем production deploy.
- **Блокеры:** внешние `OPEN-001`, `OPEN-002` и доступы владельца.
- **Принятие:** real content/media inventories сверены, deployment успешен, migrations подтверждены, secrets не раскрыты, dashboard закрыт.
