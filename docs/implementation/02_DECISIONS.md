...
| DEC-006 | Использовать существующие PostgreSQL и Azure Blob данные | Сохранение реального контента | Пустая БД; тестовые данные | Нужны backup, inventory и безопасная non-production копия | Ответ 5A | superseded by DEC-017 |
...
| DEC-017 | Для локальных milestones использовать PostgreSQL/Azurite test corpus; перенос реальных data/media отложить до M5 | Старая БД удалена, Azure account намеренно ограничен, владелец разрешил полностью локальную работу | Ждать восстановления cloud; блокировать local milestones | Локальные проверки не доказывают production content, поэтому M5 сохраняет отдельный real-data gate | Явное подтверждение владельца 13 июля 2026 | accepted; supersedes DEC-006 for M1-M4 |
