...
**Состояние:** планирование завершено; `BASE-01` выполнена; изменения исходного кода ещё не начинались.

## Назначение

Каталог содержит утверждённый план доведения существующего сайта музыкальной группы до проверяемого завершения и результаты выполненной документальной задачи `BASE-01`. Исходный код пока не изменялся.
